...
from activity_recognition.utils.metrics import compute_metrics, save_json
from activity_recognition.utils.plotting import save_confusion_matrix


def evaluate_from_config(config_path: str | Path, run_dir: str | Path) -> dict[str, object]:
    ...
    class_names = json.loads((run_dir / "label_classes.json").read_text(encoding="utf-8"))
    preprocessing = json.loads((run_dir / "preprocessing.json").read_text(encoding="utf-8"))
    split_subjects = json.loads((run_dir / "split_subjects.json").read_text(encoding="utf-8"))
...
def _encode_labels(labels: np.ndarray, class_names: list[str]) -> np.ndarray:
    ...
    if unknown:
        raise ValueError(f"Found labels not present in saved label_classes.json: {unknown}")
